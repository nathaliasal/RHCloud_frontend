...
export function ContratoPlantillaStep3({ contractId, onBack, onNext, onFinish }) {
  ...
  const { data: deduccionesData, isLoading: deduccionesLoading } = useQuery({
    queryKey: ['deducciones', contractId],
  ...
  const dedMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['deducciones', contractId] })
    },
    onError: (error) => {
      setDedError(normalizeError(error))
    },
  })
  ...
  const onDeduccionSubmit = (values) => {
    ...
    if (editingDeduccion) {
      ...
      dedMutation.mutate({ id: null, body: { ...baseBody, established_contract_id: contractId } })
    }
  }
      ...
      <div className="cp-modal__body">
        <div className="cp-modal__header">
          ...
          <p className="cp-modal__sub">
            Configure las deducciones preestablecidas para el contrato (ID {contractId}). Puede agregar múltiples deducciones antes de continuar.
          </p>
        </div>
        ...
        <div className="cp-modal__form">
          ...
          <form id="cp-step3-ded" onSubmit={handleDedSubmit(onDeduccionSubmit)} className="cp-inc-form">
            <div className="cp-form__grid cp-form__grid--4">
              ...
              <div className="cp-form__field">
                ...
                <input
                  type="date"
                  className={`cp-form__input${dedErrors.start_date ? ' cp-form__input--error' : ''}`}
                  {...regDed('start_date')}
                />
                {dedErrors.start_date && <span className="cp-form__error">{dedErrors.start_date.message}</span>}
              </div>
              ...
              <div className="cp-form__field">
                ...
                <input
                  type="date"
                  className={`cp-form__input${dedErrors.end_date ? ' cp-form__input--error' : ''}`}
                  {...regDed('end_date')}
                />
                {dedErrors.end_date && <span className="cp-form__error">{dedErrors.end_date.message}</span>}
              </div>

              <div className="cp-form__field">
                <label className="cp-form__label">Descripción</label>
                <input
                  className={`cp-form__input${dedErrors.description ? ' cp-form__input--error' : ''}`}
                  placeholder="Descripción de la deducción"
                  {...regDed('description')}
                />
                {dedErrors.description && <span className="cp-form__error">{dedErrors.description.message}</span>}
              </div>
              ...
              <div className="cp-form__field">
                ...
                <input
                  ...
                  min="0"
                  step="0.01"
                  className={`cp-form__input${dedErrors.amount ? ' cp-form__input--error' : ''}`}
                  placeholder="0"
                  {...regDed('amount')}
                />
                {dedErrors.amount && <span className="cp-form__error">{dedErrors.amount.message}</span>}
              </div>
            </div>
            ...
            <div className="cp-inc-form-actions">
              ...
              <button
                ...
                disabled={dedMutation.isPending}
              >
                {dedMutation.isPending
                  ? 'Guardando...'
                  : editingDeduccion
                  ...
                  <div className="cp-inc-item__top">
                    ...
                    <div className="cp-actions">
                      <button
                        ...
                        onClick={() => startEditDeduccion(ded)}
                      ...
                      <button
                        ...
                        onClick={() => deleteMutation.mutate(ded.id)}
                        disabled={deleteMutation.isPending}
                      >
                        Eliminar
                      </button>
