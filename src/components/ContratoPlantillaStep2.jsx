...
export function ContratoPlantillaStep2({ contractId, onBack, onNext, onFinish }) {
  ...
  const { data: incrementosData, isLoading: incrementosLoading } = useQuery({
    queryKey: ['incrementos', contractId],
  ...
  const incMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['incrementos', contractId] })
    },
    onError: (error) => {
      setIncError(normalizeError(error))
    },
  })
  ...
  const onIncrementoSubmit = (values) => {
    ...
    if (editingIncrement) {
      ...
      incMutation.mutate({ id: null, body: { ...baseBody, established_contract_id: contractId } })
    }
  }
      ...
      <div className="cp-modal__body">
        <div className="cp-modal__header">
          ...
          <p className="cp-modal__sub">
            Configure los incrementos preestablecidos para el contrato (ID {contractId}). Puede agregar múltiples incrementos antes de continuar.
          </p>
        </div>
        ...
        <div className="cp-modal__form">
          ...
          <form id="cp-step2-inc" onSubmit={handleIncSubmit(onIncrementoSubmit)} className="cp-inc-form">
                  ...
                  <div className="cp-inc-item__top">
                    ...
                    <div className="cp-actions">
                      <button
                        ...
                        onClick={() => startEditIncremento(inc)}
